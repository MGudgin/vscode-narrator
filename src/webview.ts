...
export function renderMarkdown(webview: vscode.Webview, markdown: string): string {
    return wrap(webview, md.render(markdown));
}

export function renderLoading(webview: vscode.Webview, fileLabel: string): string {
    const body = `<h1>Narrating <code>${escapeHtml(fileLabel)}</code>…</h1>
<p class="status">Calling the language model. This may take a few seconds.</p>`;
    return wrap(webview, body);
}

export function renderError(webview: vscode.Webview, message: string, hint?: string): string {
    const hintHtml = hint ? `<p>${escapeHtml(hint)}</p>` : '';
    const body = `<h1 class="error">Narration failed</h1>
